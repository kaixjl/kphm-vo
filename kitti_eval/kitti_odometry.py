...
class KittiEvalOdom():
    ...
    def eval_pose(self, gt, result, result_dir, alignment=None):
        # type: (dict[int, dict[int, np.ndarray]], dict[int, dict[int, np.ndarray]], str, str) -> None
        """Evaulate required/available sequences
        Args:
            gt: {seq, {idx: np.ndarray with 4x4}}, got by KittiEvalOdom.load_poses_from_txt()
            result: {seq, {idx: np.ndarray with 4x4}}, got by KittiEvalOdom.load_poses_from_txt()
            result_dir: dir to which the result print
        ...
        self.eval_seqs = list(result.keys())
